...
class WorldModelEnv:
    ...
    def reset(self) -> torch.FloatTensor:
        ...
        obs = (
            torchvision.transforms.functional.to_tensor(self.env.reset())
            .to(self.device)
            .unsqueeze(0)
        )  # (1, C, H, W) in [0., 1.]
    ...
    def reset_from_initial_observations(
        ...
        return self.tokenizer.decode_obs_tokens(self.obs_tokens)

    @torch.no_grad()
    def refresh_keys_values_with_initial_obs_tokens(
        self, obs_tokens: torch.LongTensor
    ) -> torch.FloatTensor:
    ...
    def step(
        ...
        for k in range(num_passes):  # assumption that there is only one action token.
            ...
            if k == 0:
                reward = (
                    Categorical(logits=outputs_wm.logits_rewards)
                    .sample()
                    .float()
                    .cpu()
                    .numpy()
        ...
        obs = (
            self.tokenizer.decode_obs_tokens(self.obs_tokens)
    ...
    def render_batch(self) -> List[Image.Image]:
        frames = self.tokenizer.decode_obs_tokens(self.obs_tokens).detach().cpu()
        frames = rearrange(frames, "b c h w -> b h w c").numpy().astype(np.uint8)
