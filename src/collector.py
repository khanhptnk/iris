...
class Collector:
    ...
    def collect(
        ...
        *,
        ...
        should_stop = (
            lambda steps, episodes: steps >= num_steps
            if num_steps is not None
            else episodes >= num_episodes
        )
        ...
        if set(self.episode_ids) != {None} and burn_in > 0:
            current_episodes = [
                self.dataset.get_episode(episode_id) for episode_id in self.episode_ids
            ]
            segmented_episodes = [
                episode.segment(
                    start=len(episode) - burn_in, stop=len(episode), should_pad=True
                )
                for episode in current_episodes
            ]
            mask_padding = torch.stack(
                [episode.mask_padding for episode in segmented_episodes], dim=0
            ).to(agent.device)
            burnin_obs = (
                torch.stack(
                    [episode.observations for episode in segmented_episodes], dim=0
                )
                .float()
                .to(agent.device)
            )
        ...
        while not should_stop(steps, episodes):
            ...
            if self.env.should_reset():
                ...
                for episode_id in self.episode_ids:
                    ...
                    to_log.append(
                        {
                            f"{self.dataset.name}/{k}": v
                            for k, v in metrics_episode.items()
                        }
                    )
        ...
        metrics_collect = {
            f"{self.dataset.name}/{k}": v for k, v in metrics_collect.items()
        }
    ...
    def add_experience_to_dataset(
        ...
        for i, (o, a, r, d) in enumerate(
            zip(
                *map(
                    lambda arr: np.swapaxes(arr, 0, 1),
                    [observations, actions, rewards, dones],
                )
            )
        ):  # Make everything (N, T, ...) instead of (T, N, ...)
            episode = Episode(
                observations=torch.LongTensor(o)
                .permute(0, 3, 1, 2)
