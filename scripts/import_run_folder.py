...
def main() -> None:
    ...
    parser.add_argument(
        "src_folder",
        type=str,
        help="Folder to import. Must be formatted as: USERNAME@HOSTNAME:PATH_TO_OUTPUTS_FOLDER/DATE/TIME",
    )
    ...
    if args.from_key:
        subprocess.run(
            f"ln -s {str(dst_folder)} {args.src_folder}", shell=True, check=True
        )

    download_media = args.verbose and input("Download media/ ? [y/N] ").lower() == "y"
    download_last = (
        not args.verbose
        or input("Download checkpoints/last.pt ? [Y/n] ").lower() != "n"
    )
    ...
    for folder in folders:
        subprocess.run(
            f"scp -r {src_folder}/{folder} {dst_folder}", shell=True, check=True
        )
    ...
    if download_last:
        ...
        subprocess.run(
            f"scp {src_folder}/checkpoints/last.pt {checkpoint_folder}",
            shell=True,
            check=True,
        )
